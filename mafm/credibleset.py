...
def combine_creds(
    creds: List[CredibleSet], combine_cred: str = "union", combine_pip: str = "max", jaccard_threshold: float = 0.1
) -> CredibleSet:
    """
    ...
    if combine_cred == "union":
        ...
        for cred in creds:
            ...
            merged_snps.extend(snps)
        merged_snps = [list(set(merged_snps))]
    elif combine_cred == "intersection":
        for i, cred in enumerate(creds):
            snps = [i for snp in cred.snps for i in snp]
            if i == 0:
                merged_snps = set(snps)
            else:
                merged_snps.intersection_update(set(snps))
        if len(merged_snps) == 0:
            logger.warning("No common SNPs found in the intersection of credible sets.")
        merged_snps = [list(merged_snps)]
...
def create_similarity_matrix(dict_sets: List[List[Dict[str, float]]]) -> Tuple[np.ndarray, List[Dict[str, float]]]:
    """
...
def cluster_cs(dict_sets: List[List[Dict[str, float]]], threshold: float = 0.9) -> List[List[str]]:
    """
    ...
    return [list(set(cluster_groups[cluster_id])) for cluster_id in sorted(cluster_groups)]
