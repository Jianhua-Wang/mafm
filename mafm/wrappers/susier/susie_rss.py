...
logger = logging.getLogger("SuSiER")
...
def susie_suff_stat(
    ...
    if maf is not None:
        if len(maf) != len(Xty):
            raise ValueError(
                f"The length of maf does not agree with expected {len(Xty)}"
            )
    ...
    if check_input:
        ...
        if not np.allclose(Xty, projection, atol=r_tol):
            logger.warning(
                "Xty does not lie in the space of the non-zero eigenvectors of XtX"
            )
